...
while repeticao == True:
    ...
    if temperatura != int(temperatura):
        print("Por favor, insira um número inteiro válido para a temperaturaeratura.")
        continue

    temperaturaeraturas.append(temperatura)
...
if len(temperaturaeraturas) >= num_semanas * num_dias:
    temperaturaeraturas_matriz = []
    idx = 0
    for _ in range(num_semanas):
        semana = []
        for _ in range(num_dias):
            semana.append(temperaturaeraturas[idx])
            idx += 1
        temperaturaeraturas_matriz.append(semana)

    #mostra a matriz de temperaturas
    for semana, dias in enumerate(temperaturaeraturas_matriz, start=1):
        print(f"Semana {semana}: {dias}")

    print("\nMaior e menor temperaturaeratura de cada semana:")
    #mostra a maior e menor temperatura semanal 
    for i, semana in enumerate(temperaturaeraturas_matriz, start=1):
        maior = max(semana)
        menor = min(semana)
        print(f"\nSemana {i} - Maior temperatura: {maior}°C, Menor temperatura: {menor}°C")
else:
    print("\nNão há dados suficientes para formar uma matriz 2x5 de temperaturaeraturas.")
